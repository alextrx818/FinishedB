#!/usr/bin/env python3
# ⚠️ IMPORTANT: This script must ONLY be run with 'python3 live.py' command directly (NOT through systemd/supervisord).
# ⚠️ Please review docs/live-policy.md for all critical init & change-doc requirements.

# =====================================================================
# RECOMMENDED PERFORMANCE SETTINGS (2025-05-11)
# =====================================================================
# For optimal performance and to prevent VS Code disconnections,
# always run with these environment variables:
#
# SPORTS_BOT_DB_CONCURRENCY=2
# SPORTS_BOT_ALERT_CONCURRENCY=4  # Increased to 4 for multiple alert modules
# SPORTS_BOT_CONN_LIMIT=5
# SPORTS_BOT_HOST_LIMIT=2
# SPORTS_BOT_GC_TUNING=true
#
# IMPORTANT 2025-05-11: These settings are REQUIRED when using Python 3.12.3
# to prevent memory issues. Python 3.12.3 has different memory management
# characteristics and will run out of memory without these optimizations.
#
# EXAMPLE COMMAND:
# cd /root/CascadeProjects/sports_bot/football && \
#   SPORTS_BOT_DB_CONCURRENCY=2 \
#   SPORTS_BOT_ALERT_CONCURRENCY=4 \
#   SPORTS_BOT_CONN_LIMIT=5 \
#   SPORTS_BOT_HOST_LIMIT=2 \
#   SPORTS_BOT_GC_TUNING=true \
#   python3 live.py
...
# Counter for memory optimization
_fetch_cycle = 0
...
# Module-level counter for alternating print cycles and memory optimization
_fetch_cycle = 0
...
# Print cycle counter to reduce memory usage
_fetch_cycle = 0
    ...
    try:
        ...
        if hasattr(ALERT_SEMAPHORE, '_value') and ALERT_SEMAPHORE._value > 0:
            ...
            try:
                ...
                print(f"\n===== COMPLETED PROCESSING ALL {matches_processed} MATCHES (CYCLE #{_fetch_cycle}) =====\n")
...
# System verification removed (05/11/2025)
# Previous system verification code has been removed as part of system cleanup
...
# pnts3_start removed - no longer used
...
try:
    ...
    print("✓ Alert system modules imported successfully")
except ImportError as e:
    ALERT_SYSTEM_AVAILABLE = False
    print(f"⚠️ Alert system import failed: {e}")
    print("⚠️ Running without original alert system capability")
    
# Import if available: New match alerts system
try:
    from all_alerts.live_match_alerts import main_match_alerts
    MATCH_ALERTS_AVAILABLE = True
    print("✓ New match alerts system imported successfully")
except ImportError as e:
    MATCH_ALERTS_AVAILABLE = False
    print(f"⚠️ New match alerts system import failed: {e}")
    print("⚠️ Running without new match alerts capability")
...
# Initialize fetch cycle counter at module level
_fetch_cycle = 0
...
# Function to monitor system resources and performance in real-time
# Diagnostic monitor has been removed to improve startup performance
# and avoid memory usage
...
async def process_live_matches_async(session, country_map):
    ...
    # Check for time-based match filtering
    current_time = get_eastern_time()
    
    # Clear screen on alternating cycles to reduce terminal output
    if _fetch_cycle % 2 == 0:
        # Clear screen but only the visible part (scrollback remains)
        os.system('cls' if os.name == 'nt' else 'clear')
        print(f"\n===== BRIEF SUMMARY CYCLE #{_fetch_cycle} AT {current_time.strftime(TIME_FORMAT)} =====\n")
    else:
        print(f"\n===== PROCESSING LIVE MATCHES AT {current_time.strftime(TIME_FORMAT)} =====\n")
    ...
    if getattr(process_live_matches_async, 'deferred_alerts', None):
        n = len(process_live_matches_async.deferred_alerts)
        print(f"\nProcessing {n} deferred match alerts...")
        alert_results = {}
        print(f"Processing {len(process_live_matches_async.deferred_alerts)} matches for alerts")
        ...
        if len(process_live_matches_async.previous_matches) > 100:
            ...
            for match_id, _ in old_matches[:-100]:
                del process_live_matches_async.previous_matches[match_id]

    # Report memory usage statistics before refreshing
    print(f"Memory stats - Previous matches: {len(process_live_matches_async.previous_matches)}, Deferred alerts: {len(process_live_matches_async.deferred_alerts)}")
